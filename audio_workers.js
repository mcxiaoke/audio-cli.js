...
function convertAudio(file, i, total, options = {}) {
  ...
  // 修复命令行报错 Error opening input files: Invalid argument
  const fileSrcArg = fileSrc.replaceAll('\\', `\\\\`)
  const fileDstArg = file.fileDstTemp.replaceAll('\\', `\\\\`)
  ...
  // args.push("-f mp4");
  args.push(fileDstArg) // 添加临时目标文件路径

  // 记录调试信息和确保输出目录存在
  log.info(i, "ffmpeg", args)
